...
def load_label(file_name):
    ...
    for i in range(len(df)):
        ten_digit = int(df['序号'][i])-1
        line_score={'immersion':{},'satisf':{},'arousal':{},'valance':{},'visual':{},'auditory':{}}
        for col in ColNames_list:
            if col.split('、')[0].isdigit():
                digit = int(col.split('、')[0])
            if "沉浸" in col:
                line_score['immersion'][digit] = int(df[col][i])
            elif "满意" in col or "兴趣" in col:
                line_score['satisf'][digit] = int(df[col][i])
            elif "唤起" in col:
...
if __name__ == '__main__':
    for exp_name in tqdm.tqdm(exp_list):
        ...
        if exp_name in bad:
            ...
            raw.info['bads'].append(bad[exp_name])
            print(raw.info['bads'])
